...
User = settings.AUTH_USER_MODEL  # Always reference user this way
 
 
class MainTask(models.Model):
    """
    High-level task created by a Manager and assigned to a Department Head.
    """
...
class SubTask(models.Model):
    """
    Sub-task derived from a MainTask.
    Created by a Department Head, assigned to an Employee (User).
    """
    class Status(models.TextChoices):
...
class Request(models.Model):
    """
    Extension or Exemption request submitted by an Employee
    regarding a SubTask, reviewed by a Department Head.
    """
    ...
    request_type = models.CharField(
        max_length=10, choices=RequestType.choices
    )
    status = models.CharField(
        max_length=10,
        choices=Status.choices,
